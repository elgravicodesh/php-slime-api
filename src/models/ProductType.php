...
Class ProductType {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select ProductType.*, ProductType.ProductTypeID as id from producttype ProductType";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into producttype(CreatedBy,DateCreated,ProductType) Values(?,now(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->ProductType]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update producttype Set DateModified=getdate(),ModifiedBy=?,ProductType=? Where ProductTypeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->ProductType,$data->ProductTypeID]);
            //Get Updated Records
            $data = $this->getList();
