...
Class ItemType {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select ItemType.*, ItemType.ItemTypeID as id from itemtype ItemType";
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update itemtype Set ItemType=? Where ItemTypeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ItemType,$data->ItemTypeID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from itemtype where ItemTypeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
