...
Class DocumentType {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select DocumentType.*, DocumentType.DocumentTypeID as id from documentType DocumentType";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into documentType(DocumentType) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update documentType Set DocumentType=? Where DocumentTypeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->DocumentType,$data->DocumentTypeID]);
            //Get Updated Records
            $data = $this->getList();
