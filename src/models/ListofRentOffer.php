...
Class ListofRentOffer {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select ListofRentOffer.*, ListofRentOffer.RentOfferID as id from listofrentoffer ListofRentOffer";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into listofrentoffer(AccountantID,Address,CreatedBy,DateCreated,DescriptionofAccommodation,Discount,DurationID,Email,EstateAgentID,EstateManagerID,LegalOfficerID,ProspectivePhoneNum,ProspectiveTenantName,PurposeofAccommodation,RefundableCautionFee,RentOfferDate,VATID,WHTID) Values(?,?,?,now(),?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update listofrentoffer Set AccountantID=?,Address=?,DateModified=now(),DescriptionofAccommodation=?,Discount=?,DurationID=?,Email=?,EstateAgentID=?,EstateManagerID=?,LegalOfficerID=?,ModifiedBy=?,ProspectivePhoneNum=?,ProspectiveTenantName=?,PurposeofAccommodation=?,RefundableCautionFee=?,RentOfferDate=?,VATID=?,WHTID=? Where RentOfferID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from listofrentoffer where RentOfferID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
