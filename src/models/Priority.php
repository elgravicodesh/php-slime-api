...
Class Priority {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Priority.*, Priority.PriorityID as id from priority Priority";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into priority(Priority) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update priority Set Priority=? Where PriorityID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Priority,$data->PriorityID]);
            //Get Updated Records
            $data = $this->getList();
