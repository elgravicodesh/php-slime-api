...
Class City {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select City.*, City.CityID as id, Lga.StateID from city City left join lga Lga on City.LgaID = Lga.LgaID ";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into City(CreatedBy,DateCreated,Lga,LgaID) Values(?,now(),?,?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Lga,@$data->LgaID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update city Set DateModified=now(),Lga=?,LgaID=?,ModifiedBy=? Where CityID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Lga,@$data->LgaID,$UserID,$data->CityID]);
            //Get Updated Records
            $data = $this->getList();
