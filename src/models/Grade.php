...
Class Grade {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Grade.*, Grade.GradeID as id from grade Grade";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into grade(CreatedBy,DateCreated,Grade) Values(?,now(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Grade]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update grade Set DateModified=now(),Grade=?,ModifiedBy=? Where GradeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Grade,$UserID,$data->GradeID]);
            //Get Updated Records
            $data = $this->getList();
