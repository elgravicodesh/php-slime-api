...
Class ItemStatus {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select ItemStatus.*, ItemStatus.ItemStatusID as id from itemstatus ItemStatus";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into itemstatus(ItemStatus) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ItemStatus]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update itemstatus Set ItemStatus=? Where ItemStatusID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ItemStatus,$data->ItemStatusID]);
            //Get Updated Records
            $data = $this->getList();
