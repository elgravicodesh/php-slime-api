...
Class Statuses {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Statuses.*, Statuses.StatusID as id from statuses Statuses";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into statuses(StatusName) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->StatusName]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update statuses Set StatusName=? Where StatusID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->StatusName,$data->StatusID]);
            //Get Updated Records
            $data = $this->getList();
