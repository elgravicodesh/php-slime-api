...
Class Year {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Year.*, Year.YearID as id from year Year ";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into year(YearName) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update year Set YearName=? Where YearID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from year where YearID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
