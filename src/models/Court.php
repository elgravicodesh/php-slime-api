...
Class Court {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Court.*, Court.CourtID as id from court Court";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into court(CourtName,CreatedBy,DateCreated) Values(?,?,now())";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->CourtName,$UserID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update court Set CourtName=?,DateModified=now(),ModifiedBy=? Where CourtID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->CourtName,$UserID,$data->CourtID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from Court where CourtID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
