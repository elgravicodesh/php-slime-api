...
Class Company {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Company.*, Company.CompanyID as id from company Company";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into company(CompanyAbbr,CompanyName,Location,LogoUrl) Values(?,?,?,?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->CompanyAbbr,@$data->CompanyName,@$data->Location,@$data->LogoUrl]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update company Set CompanyAbbr=?,CompanyName=?,Location=?,LogoUrl=? Where CompanyID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->CompanyAbbr,@$data->CompanyName,@$data->Location,@$data->LogoUrl,$data->CompanyID]);
            //Get Updated Records
            $data = $this->getList();
