...
Class ProjectStatus {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select projectstatus.*, ProjectStatus.ProjectStatusID as id from ProjectStatus";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into projectstatus(ProjectStatus) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ProjectStatus]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update projectstatus Set ProjectStatus=? Where ProjectStatusID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ProjectStatus,$data->ProjectStatusID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from projectstatus where ProjectStatusID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
