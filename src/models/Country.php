...
Class Country {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Country.*, Country.CountryID as id from country Country";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into country(CountryName,CountryNameCode,CreatedBy,DateCreated) Values(?,?,?,now())";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->CountryName,@$data->CountryNameCode,$UserID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update country Set CountryName=?,CountryNameCode=?,DateModified=now(),ModifiedBy=? Where CountryID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->CountryName,@$data->CountryNameCode,$UserID,$data->CountryID]);
            //Get Updated Records
            $data = $this->getList();
