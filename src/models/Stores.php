...
Class Stores {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Stores.*, Stores.StoreID as id from stores Stores";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into stores(Location,StoreName) Values(?,?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Location,@$data->StoreName]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update stores Set Location=?,StoreName=? Where StoreID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Location,@$data->StoreName,$data->StoreID]);
            //Get Updated Records
            $data = $this->getList();
