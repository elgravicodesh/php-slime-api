...
Class Tool {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select tool.*, Tool.ToolID as id from Tool";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into tool(Tool) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Tool]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update tool Set Tool=? Where ToolID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Tool,$data->ToolID]);
            //Get Updated Records
            $data = $this->getList();
