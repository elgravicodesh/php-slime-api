...
Class Notice {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Notice.*, Notice.NoticeID as id from notice Notice";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into notice(CreatedBy,DateCreated,Notice) Values(?,now(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Notice]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update notice Set DateModified=now(),ModifiedBy=?,Notice=? Where NoticeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Notice,$data->NoticeID]);
            //Get Updated Records
            $data = $this->getList();
