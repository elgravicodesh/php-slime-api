...
Class Equipment {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select equipment.*, equipment.EquipmentID as id from equipment";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into equipment(Equipment) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Equipment]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update equipment Set Equipment=? Where EquipmentID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Equipment,$data->EquipmentID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from equipment where EquipmentID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
