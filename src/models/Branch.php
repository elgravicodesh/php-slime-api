...
Class Branch {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Branch.*, Branch.BranchID as id from branch Branch";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into branch(Branch,CreatedBy,DateCreated) Values(?,?,now())";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Branch,$UserID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update branch Set Branch=?,DateModified=now(),ModifiedBy=? Where BranchID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Branch,$UserID,$data->BranchID]);
            //Get Updated Records
            $data = $this->getList();
