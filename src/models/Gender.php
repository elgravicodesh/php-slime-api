...
Class Gender {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Gender.*, Gender.GenderID as id from gender Gender";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into gender(CreatedBy,DateCreated,Gender) Values(?,now(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Gender]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update gender Set DateModified=now(),Gender=?,ModifiedBy=? Where GenderID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Gender,$UserID,$data->GenderID]);
            //Get Updated Records
            $data = $this->getList();
