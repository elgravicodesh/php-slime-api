...
Class ItemCategory {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select ItemCategory.*, ItemCategory.ItemCategoryID as id from itemcategory ItemCategory";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into itemcategory(ItemCategory) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ItemCategory]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update itemcategory Set ItemCategory=? Where ItemCategoryID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ItemCategory,$data->ItemCategoryID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from itemcategory where ItemCategoryID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
