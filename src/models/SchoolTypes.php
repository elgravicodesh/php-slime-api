...
Class SchoolTypes {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select SchoolTypes.*, SchoolTypes.SchoolTypeID as id from schooltypes SchoolTypes";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into schooltypes(SchoolType) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->SchoolType]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update schooltypes Set SchoolType=? Where SchoolTypeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->SchoolType,$data->SchoolTypeID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from schooltypes where SchoolTypeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
