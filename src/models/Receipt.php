...
Class Receipt {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Receipt.*, Receipt.ReceiptID as id from receipt Receipt";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into receipt(ReceiptNum) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ReceiptNum]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update receipt Set ReceiptNum=? Where ReceiptID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->ReceiptNum,$data->ReceiptID]);
            //Get Updated Records
            $data = $this->getList();
