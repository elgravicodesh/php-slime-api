...
Class Unit {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Unit.*, Unit.UnitID as id from unit Unit";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into unit(CreatedBy,DateCreated,Unit) Values(?,getdate(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Unit]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update unit Set DateModified=getdate(),ModifiedBy=?,Unit=? Where UnitID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Unit,$data->UnitID]);
            //Get Updated Records
            $data = $this->getList();
