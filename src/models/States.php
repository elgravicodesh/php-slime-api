...
Class States {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select States.*, States.StateID as id from states States";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into states(StateName) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->StateName]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update states Set StateName=? Where StateID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->StateName,$data->StateID]);
            //Get Updated Records
            $data = $this->getList();
