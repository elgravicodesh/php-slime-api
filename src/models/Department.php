...
Class Department {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Department.*, Department.DepartmentID as id from department Department";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into department(CreatedBy,DateCreated,Department) Values(?,now(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Department]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update department Set DateModified=now(),Department=?,ModifiedBy=? Where DepartmentID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Department,$UserID,$data->DepartmentID]);
            //Get Updated Records
            $data = $this->getList();
