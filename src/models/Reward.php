...
Class Reward {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select Reward.*, Reward.RewardID as id from reward Reward";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into reward(CreatedBy,DateCreated,Reward) Values(?,now(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Reward]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update reward Set DateModified=now(),ModifiedBy=?,Reward=? Where RewardID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->Reward,$data->RewardID]);
            //Get Updated Records
            $data = $this->getList();
