...
Class UserGroup {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select UserGroup.*, UserGroup.UserGroupID as id from usergroup UserGroup";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into usergroup(CreatedBy,DateCreated,UserGroup) Values(?,getdate(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->UserGroup]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update usergroup Set DateModified=getdate(),ModifiedBy=?,UserGroup=? Where UserGroupID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->UserGroup,$data->UserGroupID]);
            //Get Updated Records
            $data = $this->getList();
