...
Class LeaveType {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select LeaveType.*, LeaveType.LeaveTypeID as id from leavetype LeaveType";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into leavetype(LeaveType) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->LeaveType]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update leavetype Set LeaveType=? Where LeaveTypeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->LeaveType,$data->LeaveTypeID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from leavetype where LeaveTypeID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
