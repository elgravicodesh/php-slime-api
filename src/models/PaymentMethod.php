...
Class PaymentMethod {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select PaymentMethod.*, PaymentMethod.PaymentMethodID as id from paymentmethod PaymentMethod";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into paymentmethod(CreatedBy,DateCreated,PaymentMethod) Values(?,now(),?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->PaymentMethod]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update paymentmethod Set DateModified=now(),ModifiedBy=?,PaymentMethod=? Where PaymentMethodID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([$UserID,@$data->PaymentMethod,$data->PaymentMethodID]);
            //Get Updated Records
            $data = $this->getList();
