...
Class PaymentTranche {
    ...
    public function getList($data=array())
    {
        $result =array();
        try{
            $sql ="Select PaymentTranche.*, PaymentTranche.PaymentTrancheID as id from paymenttranche PaymentTranche";
    ...
    public function add($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Insert into paymenttranche(Tranche) Values(?)";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Tranche]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function update($data)
    {
        ...
        try{
            $UserID =0;
            //Insert Query
            $sql ="Update paymenttranche Set Tranche=? Where PaymentTrancheID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            //Parameter Placeholder Assigment
            $stmt->execute([@$data->Tranche,$data->PaymentTrancheID]);
            //Get Updated Records
            $data = $this->getList();
    ...
    public function get($id)
    {
        ...
        $result =array();
        try{
            $sql ="Select * from paymenttranche where PaymentTrancheID=?";
            $db = $this->db;
            $stmt = $db->prepare($sql);
            $stmt->execute([$id]);
